      ...
      fetch(urlPlayoffs)
        ...
        .then(function (data) {
          ...
          for (let i = 0; i < idsData["cargoquery"].length; i++) {
            if (
              idsData["cargoquery"][i]["title"]["Blue"] == teams ||
              idsData["cargoquery"][i]["title"]["Red"] == teams
            ) {
              console.log("match encontrado de " + teams);
              var textSelector =
                idsData["cargoquery"][i]["title"]["DateTime UTC"].slice(0, 10) +
                "    Blue: " +
                idsData["cargoquery"][i]["title"]["Blue"] +
                "   VS" +
                "    Red: " +
                idsData["cargoquery"][i]["title"]["Red"];
              var opcion = document.createElement("option");
              opcion.text = textSelector;
              opcion.value =
                idsData["cargoquery"][i]["title"]["RiotPlatformGameId"];
              selector.add(opcion);
            }
          }
          for (let i = 0; i < idsDataPlayoffs["cargoquery"].length; i++) {
            if (
              idsDataPlayoffs["cargoquery"][i]["title"]["Blue"] == teams ||
              idsDataPlayoffs["cargoquery"][i]["title"]["Red"] == teams
            ) {
              console.log("match encontrado de " + teams);
              var textSelector =
                idsDataPlayoffs["cargoquery"][i]["title"]["DateTime UTC"].slice(
                  0,
                  10
                ) +
                "    Blue: " +
                idsDataPlayoffs["cargoquery"][i]["title"]["Blue"] +
                "   VS" +
                "    Red: " +
                idsDataPlayoffs["cargoquery"][i]["title"]["Red"] +
                " PLAYOFFS";
              var opcion = document.createElement("option");
              opcion.text = textSelector;
              opcion.value =
                idsDataPlayoffs["cargoquery"][i]["title"]["RiotPlatformGameId"];
              selector.add(opcion);
            }
          }
