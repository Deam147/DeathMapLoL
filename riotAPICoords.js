    ...
    .then(function (data) {
      ...
      for (let i = 2; i < timelineData["frames"].length; i++) {
        ...
        for (let y = 0; y < timelineData["frames"][i]["events"].length; y++) {
          ...
          if (
            timelineData["frames"][i]["events"][y]["type"] == "CHAMPION_KILL"
          ) {
            console.log(
              "found kill de: " +
                timelineData["frames"][i]["events"][y]["participantId"]
            );
            victims.push(timelineData["frames"][i]["events"][y]["victimId"]);

            if (
              timelineData["frames"][i]["events"][y]["victimId"] == playerSelect
            ) {
              var killerId = timelineData["frames"][i]["events"][y]["killerId"];

              var victimId = timelineData["frames"][i]["events"][y]["victimId"];
              var timeKill = minutesToTime(
                timelineData["frames"][i]["events"][y]["timestamp"]
              );

              var imageKiller =
                "<img class='imageChampion' src='https://ddragon.leagueoflegends.com/cdn/12.13.1/img/champion/" +
                championsNames[killerId] +
                ".png'></img>";
              var imageVictim =
                "<img class='imageChampion' src='https://ddragon.leagueoflegends.com/cdn/12.13.1/img/champion/" +
                championsNames[victimId] +
                ".png'></img>";

              //console.log("found death jungle red")
              textChampionKill =
                imageKiller +
                "<span class='champKills'>" +
                playersNames[killerId] +
                " Killed " +
                playersNames[victimId] +
                "<span>" +
                imageVictim +
                "<br>" +
                "<p class='timer'>" +
              ...
              coords.push([
                timelineData["frames"][i]["events"][y]["position"]["x"],
                timelineData["frames"][i]["events"][y]["position"]["y"],
                textChampionKill,
              ]);
            }
          }
...
async function test(coordenadas) {
  ...
  var mapCords = cords,
    ...
    championdead =
      "https://raw.communitydragon.org/12.13/game/assets/ux/minimap/icons/champion_dead.png",
    xScale,
    yScale,
    svg;
